...
// SecondaryStorageConfigReconciler reconciles a SecondaryStorageConfig object
...
// +kubebuilder:rbac:groups=core.camunda.io,resources=secondarystorageconfigs/finalizers,verbs=update

// Reconcile is part of the main kubernetes reconciliation loop which aims to
// move the current state of the cluster closer to the desired state.
// TODO(user): Modify the Reconcile function to compare the state specified by
// the SecondaryStorageConfig object against the actual cluster state, and then
...
func (r *SecondaryStorageConfigReconciler) SetupWithManager(mgr ctrl.Manager) error {
	return ctrl.NewControllerManagedBy(mgr).
		For(&corev1.SecondaryStorageConfig{}).
