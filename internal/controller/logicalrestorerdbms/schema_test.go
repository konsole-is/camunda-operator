...
// The admission schema of the LogicalRestoreRDBMS CRD.
var _ = Describe("LogicalRestoreRDBMS schema", func() {
	newSchemaRestore := func(namespace string) *v1.LogicalRestoreRDBMS {
	...
	It("rejects every change to the spec: a retry is a new resource", func() {
		lrr := newSchemaRestore(newNamespace())
		Expect(k8sClient.Create(ctx, lrr)).To(Succeed())
		DeferCleanup(func() { _ = k8sClient.Delete(ctx, lrr) })
	...
	It("serves the resource under its plural and short name", func() {
		...
		Expect(resources.APIResources).To(ContainElement(SatisfyAll(
			HaveField("Name", "logicalrestorerdbmses"),
			HaveField("ShortNames", ConsistOf("lrrdbms")),
		)))
	})
})
