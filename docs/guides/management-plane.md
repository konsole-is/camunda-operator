...
- **Move to another identity provider.** Change `spec.identityProvider`. The workloads roll into the new mode. The first administrator does not move with them: Management Identity keeps the one it started with, in its database. The login callbacks of Optimize do move: they leave the old realm before they reach the new one. Keep the old Keycloak and its administrator Secret until `OptimizeCallbacksReady` reads `Healthy` again. See [Moving the callbacks to another realm](../crds/camundamanagementcluster.md#moving-the-callbacks-to-another-realm).
