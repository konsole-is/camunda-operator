...
Sizing and the PostgreSQL version are shared too. One `DatabaseServer` carries one instance count, one volume size, and one PostgreSQL major. Every change reaches every cluster on the server at the same time. A larger `storageSize` grows the volumes in place, and it needs a StorageClass that allows expansion. New resources or a new image restart the instances, and every cluster on the server sees that restart.
