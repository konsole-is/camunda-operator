...
implementation. It includes at least the snapshot rights on the repository and the `monitor`
cluster privilege for `_nodes/stats`. The commitment of this spec is that `superuser` goes away.
...
status:
  ...
  storageSizes:                  # effective restore sizes, best effort
    elasticsearch: 50Gi
    zeebe: 20Gi
...
status:
  ...
  storageSizes:
    zeebe: 20Gi                  # effective restore size of the brokers, best effort
  completionTime: null
  observedGeneration: 1
  conditions: [...]
```
...
**Storage sizes.** When a backup leaves `Pending`, the controller records the effective restore
size of each storage-bearing component in `status.storageSizes`, so a restore can create
right-sized volumes instead of guessing. `zeebe` is the largest capacity in
`CamundaCluster.status.volumes`. `elasticsearch` (Elasticsearch kind only) is the largest node
filesystem total from `_nodes/stats/fs`, capped by the largest node used bytes times 1.5. Both
round up to the nearest 10Gi. The rule and its factor come from the production operator: 1.5
covers the Elasticsearch watermarks (allocation stops at 85% disk usage), the segment merges and
translog after a restore, and a landing point under the auto-resize threshold. The cap can only
shrink a recorded size, never grow it. Recording is best effort: a value that cannot be computed
stays unset, an already set value is kept, and no failure propagates to the backup. The
`_nodes/stats` call needs the `monitor` cluster privilege on the `camunda` user.
...
`ObjectStorageConfig` (block matching `type`, block matching `auth.type`, S3 without region or
endpoint), on the immutable `clusterRef`, and on `retained` minimums. The storage-size rule
(cap, rounding, unset on missing input) is a pure function with table tests. The
