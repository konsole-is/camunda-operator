...
Cross-namespace Secrets (decided after PR C): a `secretKeyRef` can only name a Secret in the pod's namespace, but the platform config's client secret and license (and, in principle, any binding credential or CA) can live elsewhere (`camunda-system` in the docs). The controller therefore **mirrors** every referenced Secret whose namespace differs from the cluster's into the cluster namespace, as an owned Secret named `<name>-camunda-<purpose>` (`purpose` ∈ `license`, `oidc-client`, `auth-client`, `es-credentials`, `es-ca`, `db-credentials`), copying only the referenced keys. Add to `pkg/components/camundacluster` (pure): `MirroredSecretName(cluster, purpose) string` and `MirroredSecretComponent(cluster *v1.CamundaCluster, purpose string, data map[string][]byte) (*component.Component, error)` (condition type `v1.ConditionMirroredSecretsReady = "MirroredSecretsReady"`, one component holding every mirrored Secret; it takes part in `Ready`). The pre-check reads the source through `APIReader`, and the `Input` handed to the renderer carries the **local** names (the copies of `Platform`/`Storage`/`Effective.Auth` refs are rewritten to the mirrored Secret's name and the cluster namespace), so the renderer contract from PR C stays unchanged. Same-namespace Secrets are referenced directly. Source resource versions are already in `HashInputs`, so a rotation of a source Secret re-mirrors and rolls the pods; the Secret watch of Task D2 covers the source Secrets through the platform-config index and the same-namespace rule.
