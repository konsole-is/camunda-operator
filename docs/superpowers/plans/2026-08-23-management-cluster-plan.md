...
| `management-render-core` | #187 | #188, #189, #190 | `pkg/components/camundamanagementcluster`: `Input`, `AttachedCluster`, `IdentityProvider`, `ResolveIdentityProvider`, `Build`, `Built`, `ManagementAuthSpec`, the builder list in `components.go` (Task 2.1) | pre-merge stub PR (#187 merges before wave 2 starts) |
| `management-controller-core` | #187 | #188, #189, #190 | `internal/controller/camundamanagementcluster`: `Reconciler`, `resolved`, the per-component hook points in `controller.go`, `attachment.go` with `attachedClusters` (Task 2.3) | pre-merge stub PR (#187) |
