...
- Generated Secrets: `<name>-optimize-client`, `<name>-identity-admin`, `<name>-web-modeler-pusher`, `<name>-web-modeler-cluster-<cluster uid first 8>`; keys `client-secret`, `password`, `app-id`/`app-key`/`app-secret`.
- Field managers: `camunda-operator/camundamanagementcluster` for the CR's own resources and the contract, `camunda-operator/camundamanagementcluster-attachment` for the claim annotation on a `CamundaCluster`, and `camunda-operator/camundamanagementcluster-ping` for the ping entries in its `spec.extraEnv` (two SSA applies under one manager strip each other's fields, so claim and ping never share a manager).
- Optional components are always built and feature-gated (`WithFeatureGate`/`GatedBy`) on their spec block or mode, never skipped with a nil return: ocf deletes the resources of a component that is gated off, and a Disabled component stays out of `Ready`. A nil-returning builder leaves Deployments running after the user removes the block (found on #204; applied to Console in #204 and to Keycloak in #205).
- Wave-3 rules (from the wave-2 checkpoint): package-level helpers in the render package carry the app prefix (`consoleEnv`, `consoleProbe`, `webModelerEnv`, `keycloakCR`); never reuse or generalise `baseEnv`/`databaseEnv`/`providerEnv` of Identity without renaming them. Each PR adds its own `MirrorPurpose` constants, its own golden fixture directories (`managed-keycloak/*`, `external-keycloak/*` for #188; `console/*` for #189; `web-modeler/*` for #190) and does not regenerate `oidc/*`. Each PR adds branches to `Input.workload`, `replicas`, `confighash.componentEnv`, `precheck.resolveDatabases`, `finalize`, and `attach` as needed; the orchestrator merges the one-line conflicts. A controller hook goes after `recordInitialClaim`, never between `reconcileComponents` and it. A Secret that only one component reads enters that component's config hash only (the `camundacluster` precedent), not the global `Input.HashInputs`.
