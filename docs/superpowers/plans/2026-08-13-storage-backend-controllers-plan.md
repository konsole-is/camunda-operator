...
Setup installs ECK (pinned operator manifest version, documented in the Makefile variable) and a `postgres:17` Deployment+Service with a known admin Secret. Flow assertions per issue #39, updated for what #42/#43 shipped: `ElasticsearchCluster` (1 replica, small resources) reaches `Ready: Healthy` and `status.storageSize` equals the claim size; SSC endpoint + generated credentials authenticate (curl from an in-cluster pod using the CA from `caSecretRef`); with `monitoring.serviceMonitor.enabled` the exporter reports `es_up 1` on `<name>-es-metrics:9114`; **suspend deletes the ECK CR** (`Ready: Suspended`, data PVCs survive) and resume recreates it against the same PVCs with data intact — this is the one flow only a real ECK can prove; `Database` reaches `Ready: Healthy`; both SQL users authenticate with documented privileges; CR deletion garbage-collects the bindings and Secrets (owner refs, real GC runs here) while the logical database and the ES data PVCs survive.
