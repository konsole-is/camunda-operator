...
- **Chart values for operator inputs are first-class, even past the helm plugin:** the kubebuilder helm plugin regenerates `dist/chart` from `config/`; where it cannot express a value shape the chart needs (a nested image map for the CLI image), `hack/helmcli` post-processes the generated chart as part of `make helm-generate` — tested, failing loudly if the plugin's output changes, and documented in AGENTS.md as generation, never a hand edit. Installers set `manager.cliImage.{repository,tag}` exactly like `manager.image`. (User decision, review of #85.)
- **A backup's `clusterRef` is same-namespace only.** The operator reads the cluster's Secrets, runs a Job in its namespace, and calls its management API, so the reference must stay inside the RBAC boundary the CR itself lives in; `ClusterRef` has no `namespace` field. (User decision after Copilot flagged the ungranted cross-namespace path on #85; applied to both kinds.)
