...
| #368 (convergence) | test/suspend-only-for-shared-backends--e2e-and-convergence | .claude/worktrees/suspend-only-for-shared-backends/.claude/worktrees/e2e-and-convergence | — → fix/suspend-only-for-shared-backends | in-progress: one e2e flow for the claim and the kept workloads, four test renames, ConditionTypes on the Optimize components package, two endpoint assertions |
...
2. Checkpoint done (see the 2026-09-14 checkpoint entry). Running: the convergence sub-PR (row above). Then the spec refresh, the epic and sub-issue reconciliation (drafts under the scratchpad `checkpoint/`), and the integration PR. Gates done at af55f59: `make test` (every package ok with the envtest assets), `go -C api test ./...` ok, `make lint` 0 issues in both modules, `make manifests generate` with a clean porcelain, `go vet -tags=e2e ./test/e2e/` silent, `mkdocs build --strict` built. Running: the wave checkpoint agent. Then `feature-dev-workflow:reviewing-feature-progress` at the wave boundary, then the integration PR from `fix/suspend-only-for-shared-backends` to `main` with `Closes #368`; the Copilot auto-review fires on it, budget one fix round; teardown of the plan and this file once CI is green; the user merges. Superseded text of the earlier steps: #371 resolves the two shared test files and the pre-check failure branch of `internal/controller/camundacluster/controller.go` (#369 stages `WaitingForHandover` from the render path, #370 adds the explicit-suspend scale on the failure branch), adds the contract-deleted Lease assertion, fixes the `Suspended` GoDoc in `pkg/components/camundaoptimize/input.go:37` (it names one operator-driven state and says "storage contract"), and re-runs the gates before its self-merge. Superseded: the merge-forward is done (51fa307), and the follow-ups it named are filed as #373 (read-once suspension state), #374 (Optimize Reconcile harness), #375 (schedule skips a not-ready cluster), #376 (restore invisible to the handover gate), #377 (docs sentence sweep).
