...
| #237 | ci/cnpg-database-server--e2e | .claude/worktrees/cnpg-database-server/.claude/worktrees/e2e | #244 → feat/cnpg-database-server | in-progress (head bef2d91, round 4 + E2E CI running) |
| #243 | ci/cnpg-database-server--split-e2e-by-backend | .claude/worktrees/cnpg-database-server/.claude/worktrees/split-e2e-by-backend | → feat/cnpg-database-server | implemented locally (d1f9818, 97a25d7 on top of 1fbf09b; replay onto the feature branch after #244 merges, then open) |
...
Session burst ended 2026-08-24 ~20:56Z with #244 on head `bef2d91`, two waits armed, and #243 implemented locally.

1. **#237 / PR #244** (`ci/cnpg-database-server--e2e`, worktree `.claude/worktrees/e2e`, head `bef2d91`, 24 commits, pushed, body reconciled 20:55Z). Rounds 1-3 triaged (2 clean, 3 one rejected finding), spec pass and quality pass folded, three e2e findings root-caused and fixed. Waiting on: (a) Copilot round 4, requested 20:54:09Z on `bef2d91` (if it never lands after two 30-minute waits, the user chose "merge on CI green" for #242 in that situation); (b) `E2E Tests` on `bef2d91` (on red: read the `Camunda 8.9` job log, root-cause, never re-run; still unproven on CI: `archives the recovered cluster and reports Ready Healthy again`). Then: fold findings, one more Copilot round if any code changed, list the Copilot reviews once more right before merging, self-merge (squash), `gh issue close 237`, remove the `e2e` sub-worktree (`chmod -R u+w` its `bin/` first), update this table.
2. **#243** (`ci/cnpg-database-server--split-e2e-by-backend`, sub-worktree `.claude/worktrees/split-e2e-by-backend`, commits d1f9818 + 97a25d7 on top of #244's `1fbf09b`, unpushed): after #244 squash-merges, replay the two commits onto the feature branch (`git rebase --onto feat/cnpg-database-server 1fbf09b <branch>`, run from the sub-worktree; the hook rejects the word in a Bash command, so put it in a script file), run `go test ./test/utils/...` + `go vet -tags=e2e ./test/e2e/`, push, open the PR `ci(e2e): run one job per storage backend` with `Towards #243` (three matrix entries elasticsearch/postgres/management, `ECK_INSTALL_SKIP` wired, labels in `test/utils.AllLabels`, guard `TestWorkflowJobsClaimEveryLabel`), Copilot loop, check the three jobs' logs for the skipped installs, self-merge, `gh issue close 243`, remove the sub-worktree.
