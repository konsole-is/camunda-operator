...
| #237 | ci/cnpg-database-server--e2e | .claude/worktrees/cnpg-database-server/.claude/worktrees/e2e | #244 → feat/cnpg-database-server | in-progress (head a656a2c, round 2 + E2E CI + quality pass running) |
...
Session burst ended 2026-08-24 ~18:45Z with #244 on head `a656a2c` and three waits armed.

1. **#237 / PR #244** (`ci/cnpg-database-server--e2e`, worktree `.claude/worktrees/e2e`, head `a656a2c`, 14 commits, pushed, body reconciled 18:44Z). Round 1 is triaged, both Copilot threads replied and resolved. Waiting on: (a) Copilot round 2, requested 18:40:39Z on `a656a2c` (watermark in the session scratchpad; if it never lands after two 30-minute waits, the user chose "merge on CI green" for #242 in that situation); (b) `E2E Tests` on `a656a2c` (on red: read the `Camunda 8.9` job log, root-cause, never re-run); (c) an opus read-only quality pass on `a656a2c` (destructive paths in `recovery.go`, the `archiving` gate, mutation checks). Then: fold findings, one more Copilot round if any code changed, list the Copilot reviews once more right before merging, self-merge (squash), `gh issue close 237`, remove the `e2e` sub-worktree (`chmod -R u+w` its `bin/` first), update this table.
2. **#243** (`ci/cnpg-database-server--split-e2e-by-backend`): branch after #237 merges; PR 6 in the plan. Labels per job are in the issue body; the workflow at `a656a2c` already carries `cnpgInstallSkip` per matrix entry, read it before designing the split. Both PostgreSQL orchestration clusters stay on one node in the postgres job.
