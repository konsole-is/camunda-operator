...
1. Wave-3 checkpoint (coherence audit of #193+#200+#203+#204+#205 at d10e55d) is running; its findings become bubble-up entries, a follow-up sub-PR on the feature branch, or notes in the wave-4 dispatch prompts.
2. Dispatch wave 4 in parallel from the nested worktrees that already exist at d10e55d: #191 `e2e-flows` (plan "#191", branch `test/management-cluster--e2e-flows`) and #192 `user-docs` (plan "#192", branch `docs/management-cluster--user-docs`). Per PR: copilot-review-loop (3-round cap; the Monitor + `scratchpad/await-review.sh` wait; two timed-out waits end the loop) → spec pass → quality pass → squash-merge → `gh issue close` → state row. #191 runs its flows on a kind cluster (`KIND_CLUSTER`, detached) and pastes the output in the PR.
3. Final `reviewing-feature-progress` on the whole feature branch; integration PR `feat/management-cluster` → `main` with `Closes #185`; copilot-review-loop on it; teardown of plan and state after CI is green (spec stays); report ready-to-merge. The merge to main is the user's.
