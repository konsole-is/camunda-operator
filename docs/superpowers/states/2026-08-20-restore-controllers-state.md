...
- NEXT ACTION: run a BALANCED Copilot review loop on #136 and on #135 until each comes back clean. Then merge both into `feat/restore-controllers`. Then a quality and clarity sweep over the merged restore code. Then #113 (e2e, both new kinds) and any filed follow-up worth pulling in. Then the integration PR `feat/restore-controllers` -> main with `Closes #109`, left OPEN for the user.
...
- OPEN DECISIONS, none blocking, all inherited from the workers' reports:
    1. Issue #130 and the plan's PR-C compatibility table both say "the partition counts match" for the relational path. A relational backup records NO partition count. The code correctly omits the check. Correct the issue and the plan.
    2. `RestoreUIDLabel` sits in `pkg/components/logicalrestorerdbms` per the plan's contract row, while the Conventions section says no package declares its own label string. Decide whether to hoist it into `pkg/labels`.
    3. Both workers independently retuned the suite's `retryInterval`, to different values, because a `ClusterClaimed` hold is woken by the retry timer and by no watch. Pick one retuning at the rebase. The underlying property is real and is worth a line in the docs.
    4. `make setup-envtest` is required before `go test ./...` and appears in neither CLAUDE.md nor the plan's gate list. Every envtest suite fails without `KUBEBUILDER_ASSETS`. Worth adding to the documented gate.
