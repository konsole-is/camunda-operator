...
1. Phase 2 in flight. #67 → PR #77 open but UNVERIFIED (`make all`, `make helm-verify`, full `go test` never completed under machine thrash) — re-run all three on a quiet machine BEFORE reviewing it; treat its green claim as unproven until then. #66 resumed after stalling; it was mid-fix on a credentials-component test that now previews two objects instead of one.
2. Then per PR: two-stage orchestrator review (Copilot unavailable, see bubble-up), fix-loop to clean with no round cap, self-merge, `gh issue close <n>`, lock `snapshot-repository-field` (#66) and `management-binding` (#67).
3. #67 bubble-ups to fold in at review: `ManagementAuth` uses `Method` + `*CredentialsSecretRef` (no `SecretRef` type exists, and camundaadmin needs user+password) and publishes method `none` (the 9600 port is unauthenticated; publishing `basic` would point consumers at the port-8080 credential) — both defensible, but the `management-binding` contract row must be amended to match before #68/#69 build against it. `spec.backup.dump` is carried through the API and merge but renders nothing until #69. ServiceAccount `name`/`create` on the CamundaCluster side is follow-up once #66 lands the shared-type fields.
