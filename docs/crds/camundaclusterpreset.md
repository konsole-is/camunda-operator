...
- The `scheduling` blocks are not schema-validated at admission, like on the CamundaCluster. Keys outside `nodeAffinity`, `podAffinity`, and `tolerations` are ignored. An invalid value surfaces on the process condition of a consuming cluster.
