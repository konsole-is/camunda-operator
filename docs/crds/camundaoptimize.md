...
spec:
  ...
  managementAuthRef: management-auth
  # string. Optional. The URL a browser reaches this Optimize at. It carries no comma. In the two Keycloak modes the management plane registers the login callback under it. Unused in the oidc mode.
  externalUrl: "https://optimize.camunda.example.com"
...
- `spec.externalUrl` must be an `http` or `https` URL, and it carries no comma.
