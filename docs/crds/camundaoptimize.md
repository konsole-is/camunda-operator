...
| `Ready` | `InvalidReference` | The `clusterRef`, the `managementAuthRef`, or the `storageRef` chain of the cluster does not resolve. It also reports a referenced cluster whose effective spec is invalid, such as a version below `8.9.0`. The operator removes both workloads of a `CamundaCluster` that does not exist any more, and builds them again when that cluster returns. | Read the message. Create the missing resource, or correct the field it names. |
