...
| `Ready` | `ClusterSuspended` | The cluster is suspended, by `spec.suspend` or by the operator while another cluster holds its backend. The backup waits. | Read the `Ready` condition of the cluster. Set `spec.suspend` to `false`, or remove what suspends it. |
