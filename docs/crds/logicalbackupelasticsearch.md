...
**Exporting.** The backup pauses exporting before it writes and resumes it at the end. Records still flow into Elasticsearch, but Zeebe cannot compact its log while exporting is paused. If a step fails, the backup resumes exporting before it ends as `Failed`.
...
Realistic, with a name that carries the date:
