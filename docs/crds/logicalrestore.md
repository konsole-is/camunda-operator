...
4. Enter `RestoringSecondaryStorage`:
    - Elasticsearch: delete the existing Camunda indices in the target's Elasticsearch, then restore every snapshot of the backup — the web-application parts in their original numbered order, then the Zeebe record snapshot — directly through the Elasticsearch snapshot API. Camunda exposes no restore endpoint, so the operator talks to Elasticsearch itself using the credentials from the target's `SecondaryStorageConfig`. The index and component templates survive because they were seeded when the target cluster first started.
    - RDBMS: create a Job that downloads the dump from the backup bucket and runs `pg_restore` against the target's logical database, resolved via the target's `storageRef` → `SecondaryStorageConfig` → `DatabaseConfig` and authenticated with its `backupCredentialsSecretRef`. All Jobs this controller creates are applied with Server-Side Apply (SSA) under the field manager `camunda-operator/logicalrestore`.
