...
`kubectl get backupschedules` lists the schedules with `Ready`, its reason, the cron expression, and the age. `kubectl get backupschedules -o wide` adds the cluster, the last trigger, and the last backup.
...
`kubectl get backupschedules -n my-cluster-ns -o wide` then shows the cron expression, the cluster, the last trigger, and the last backup that the schedule created.
