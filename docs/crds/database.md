...
If `spec.serverRef` names no `DatabaseServerConfig`, `Ready` is `False` with reason `InvalidReference`. If the admin credentials Secret of the server is missing or lacks a key, the reason is `MissingSecret`. If the server does not answer or rejects the admin credentials, the reason is `ConnectionFailed` and the operator retries every 30 seconds.

## Uniqueness

One `Database` owns one pair of `serverRef` and `databaseName`. If a second `Database` claims the same pair, the oldest one wins. On an equal creation timestamp, the smaller name wins. The loser reports `InvalidReference`, runs no SQL, and publishes nothing.
