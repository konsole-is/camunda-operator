...
4. Unless `spec.backupCredentials.disabled` is `true`, it creates a separate backup user granted dump/restore privileges (`pg_dump`/`pg_restore`) and writes its credentials to the Secret named by `spec.backupCredentials`.
