...
4. It renders a ServiceAccount for the Elasticsearch pods when `spec.serviceAccount` is set, or when `spec.snapshotStorageRef` names a bucket whose contract carries a workload identity. The ServiceAccount is named `spec.serviceAccount.name`, or `<name>-es` when that is unset, and it carries the identity annotation derived from the bucket contract with `spec.serviceAccount.annotations` layered over it, so a value you state wins over the derived one on the same key. With `spec.serviceAccount.create: false` the ServiceAccount is one you manage: the operator neither creates, annotates, nor owns it, and a missing one fails the pre-check with `InvalidReference` rather than leaving every pod unschedulable. The pods run under that ServiceAccount either way, through the ECK podTemplate. For an `S3` or `GCS` bucket that carries a workload identity, the pod template also mounts the ServiceAccount token, because IRSA and Workload Identity Federation read that token and ECK mounts none unless the pod template asks for one. An `AzureBlob` container reads a projected federated token instead, which item 6 describes.
5. It provisions a dedicated Camunda user through ECK's file realm, generates credentials for it, and stores them in the Secret `<name>-es-user` that it owns (keys `username`, `password`, and `roles`). The user holds the custom role `camunda`, which the operator defines in the Secret `<name>-es-roles` and passes to ECK through `spec.auth.roles`. The role grants the cluster privileges `monitor`, `manage_index_templates`, `manage_ilm`, `manage_pipeline`, `create_snapshot`, `monitor_snapshot`, and `manage` — all but the last are what Camunda documents as required. `manage` is one deliberate widening: deleting a snapshot, which the backup finalizer does, has no narrower named privilege in Elasticsearch (`create_snapshot` covers create, list, and view only). The index privileges are `create_index`, `delete_index`, `read`, `write`, `manage`, and `manage_ilm`. The role does not grant `cluster:admin/repository`, so the Camunda user cannot register a snapshot repository; the operator does that with the `elastic` user of ECK instead. The operator generates the password once and keeps it stable across reconciles. To rotate it, delete the `<name>-es-user` Secret: the operator generates a new password on the next reconcile, and never republishes the old one onto the new Secret.
6. When `spec.snapshotStorageRef` is set, it owns the whole Elasticsearch side of that bucket. It serves all three storage types of the contract. For a contract with `auth.type: credentials` it renders the Secret `<name>-es-snapshot-keystore` and passes it to ECK ahead of `spec.secureSettings`. Elasticsearch reads the credentials of a repository from the node keystore alone, never from the settings of the repository. The entries of that Secret follow the storage type: `s3.client.default.access_key` and `s3.client.default.secret_key` for `S3`, `gcs.client.default.credentials_file` with the whole service-account JSON key for `GCS`, and `azure.client.default.account` with `azure.client.default.key` for `AzureBlob`. For a contract with `auth.type: workloadIdentity` the nodes authenticate as their ServiceAccount. An `S3` or `GCS` bucket then needs no keystore Secret at all. An `AzureBlob` container keeps one that holds `azure.client.default.account` alone, because the account is the one mandatory setting of an azure client under every authentication choice. Azure workload identity needs more than the ServiceAccount: the operator puts the label `azure.workload.identity/use: "true"` on the pods, without which the webhook of Azure injects nothing, and projects the federated token into `/usr/share/elasticsearch/config/azure/tokens` with `AZURE_FEDERATED_TOKEN_FILE` pointing at it, because Elasticsearch reads files under its own configuration directory alone. For an `AzureBlob` contract the operator also derives the endpoint suffix of the azure client from the `azureBlob.endpoint` of the bucket contract and writes it to the node configuration, because the keystore does not take it; Elasticsearch addresses an account as `https://<account>.blob.<suffix>` and configures the suffix alone, so an endpoint that does not reduce to one — a path-addressed emulator such as Azurite, for example — fails the pre-check with `InvalidReference` instead of registering against the public endpoint of the account. It then registers the snapshot repository `<name>` in Elasticsearch, authenticated as the `elastic` user of ECK, of type `s3`, `gcs`, or `azure` after the storage type of the contract, and with the base path `<basePath>/<namespace>/<name>` of the contract — the same layout every backup object key builds on. An `s3` repository also carries a `region`: the one the contract names, or the placeholder `us-east-1` when the contract addresses the bucket by endpoint and names none. That is the value every other consumer of the bucket sends. The region enters the SigV4 signature of each request, and Elasticsearch resolves one of its own when the repository names none, so a repository without it can sign for a region that the store rejects. The region is a repository setting and not a keystore entry, so a change of it needs no restart of the nodes. A repository that converged is not re-registered while its settings are unchanged, because Elasticsearch verifies a repository on every registration with a test blob from every data node; a settings change, a lost condition, or an operator restart registers again. `SnapshotRepositoryReady` reports the outcome, and a failed registration keeps `Ready` false.
7. It creates and keeps current a `SecondaryStorageConfig` named `spec.secondaryStorageConfig` in this CR's own namespace, with `type: elasticsearch`, the in-cluster HTTPS endpoint of the ECK-managed service, a reference to the generated credentials Secret, a `caSecretRef` pointing at the ECK-generated CA certificate Secret so consumers can verify the cluster's self-signed HTTPS endpoint, and, once the registration of a referenced bucket's repository has converged, `snapshotRepository` with its name — never before, so a consumer never snapshots against a repository that does not exist. The name survives a suspension: the registration persists in the cluster state that the data volumes retain. The `CamundaCluster` reads that field and configures its components with it; it performs no Elasticsearch administration of its own.
...
| Type | Reason | Meaning |
| --- | --- | --- |
| `Ready` | `InvalidReference` | `spec.presetRef` does not resolve to an existing `ElasticsearchClusterPreset`, or the merged spec is incomplete or below the version floor. It also covers the bucket: `spec.snapshotStorageRef` does not resolve to an existing `ObjectStorageConfig`, or the contract it names carries settings that Elasticsearch cannot express, such as an `azureBlob` endpoint that is not an endpoint suffix. |
...
| `CredentialsReady`, `KeystoreReady`, `ElasticsearchReady`, `StorageContractReady` | component status | The operational detail of the component framework for each component that makes up `Ready`. `KeystoreReady` is `Disabled` unless a referenced bucket needs keystore entries. An `S3` or `GCS` bucket needs them only with static credentials. An `AzureBlob` container always needs them, because its storage account is a keystore entry. |
