...
| `Ready` | `ClusterSuspended` | The cluster is suspended, by `spec.suspend` or by the operator. The operator suspends it while another cluster holds its backend, or while it waits for the pods of a previous holder to leave that backend. The backup waits. | Read the `Ready` condition of the cluster. Set `spec.suspend` to `false`, or remove what suspends it. |
