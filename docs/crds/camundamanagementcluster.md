...
Management Identity restarts twice in the life of a management plane over this: when the first Optimize arrives and when the last one goes. Adding or removing an Optimize between those two leaves it running. The addresses live in the ConfigMap `my-management-identity-optimize-urls`, which the Identity pods read the list from, so a change to the list does not restart them. The two restarts happen because that is when Management Identity starts or stops creating the Optimize client at all.
