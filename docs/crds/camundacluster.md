...
A cluster that `spec.suspend` stopped while the check failed stays at zero until the check passes. Clearing `spec.suspend` alone does not start it, because only a pass of the check restores the replicas. `status.gateway` and `status.management` stay empty until then, the per-process conditions read `Suspended` with the message `Kept at zero until the reference check passes`, and `Ready` says the same.
