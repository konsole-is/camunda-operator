...
The condition of each stopped workload reads `Suspending` while its pods drain, then `Suspended` with the message `Kept at zero until the reference check passes`. `Ready` carries the failure message, followed by `The workloads that stopped stay at zero until the reference check passes`. `status.gateway` and `status.management` stay empty while the workload that serves them is at zero. A workload whose stop was refused keeps running, its condition says why, and the endpoints it serves stay published.
