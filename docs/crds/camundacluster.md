...
On Camunda 8.10 and later the four names are `CAMUNDA_HUB_PING_*` instead. The management plane owns these names and replaces a value you set under one of them. It removes an entry that sets `valueFrom` under one of them, because one entry cannot hold `value` and `valueFrom` together. The `CamundaManagementCluster` then records the Warning event `ConsolePingEntryRemoved`, which names the entry and the field manager that wrote it. To keep an entry of your own, give it a name that the management plane does not own.
