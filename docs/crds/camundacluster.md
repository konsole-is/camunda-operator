...
Each `extraEnv` list merges by name under server-side apply. A field manager owns only the entries that it applies. An extension controller can therefore add an entry next to yours, and neither side removes the other. One list cannot hold two entries with the same name, and the API server rejects a manifest that does. `extraEnvFrom` stays atomic, because a source carries no name to merge on.
