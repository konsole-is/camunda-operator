...
be hand-edited. Shipped defaults come from `config/`. `dist/chart/Chart.yaml`
and `dist/chart/README.md` are preserved by `--force` and *are* hand-maintained
— these two are the only `dist/` files checked into this repository
