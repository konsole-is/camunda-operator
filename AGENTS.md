...
- In this repository `dist/**/*` is gitignored (except `dist/chart/Chart.yaml`
  and `dist/chart/README.md`) — `install.yaml` is **not** committed. It is
  attached as a release asset by `.github/workflows/release.yml` instead.
- Users only need `kubectl` to install (no additional tools required)

**Example:** Users install with a single command, using the release asset URL
...
be hand-edited. Shipped defaults come from `config/`. `make helm-generate` runs
one post-processing step after the plugin, `hack/helmcli`, which turns the
manager's `CAMUNDA_OPERATOR_CLI_IMAGE` environment variable into the
`manager.cliImage.{repository,tag}` values; it is part of generation, not a
hand edit. `dist/chart/Chart.yaml` and `dist/chart/README.md` are preserved by
...
export CLI_IMG=<registry>/<project>-cli:<version>
make docker-build-cli docker-push-cli CLI_IMG=$CLI_IMG
```

The manager needs `--camunda-operator-cli-image` (env `CAMUNDA_OPERATOR_CLI_IMAGE`)
and refuses to start without it; `make deploy`, `make build-installer`, and
`make helm-deploy` stamp `CLI_IMG` into it the same way `IMG` stamps the manager
image.
