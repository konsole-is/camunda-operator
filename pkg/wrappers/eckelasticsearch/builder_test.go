...
// testObject returns a valid namespaced Elasticsearch fixture.
func testObject() *elasticsearchv1.Elasticsearch {
	return &elasticsearchv1.Elasticsearch{
...
func TestBuilderBuildValidation(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			assert.Equal(t, "elasticsearch.k8s.elastic.co/v1/Elasticsearch/test-ns/test-object", res.Identity())
		})
	}
}
...
func TestExtractIntoDeclaredExtraction(t *testing.T) {
	...
	cell := concepts.NewData[string]("eckelasticsearch-name")
	builder := NewBuilder(testObject())
	ExtractInto(builder, cell, func(o elasticsearchv1.Elasticsearch) (string, error) {
		return o.Name, nil
	})
	...
	assert.Equal(t, "eckelasticsearch-name", produced[0].Name())
