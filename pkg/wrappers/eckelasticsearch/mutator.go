// Package eckelasticsearch provides a builder and resource for managing Elasticsearch objects.
...
// Mutation defines a mutation that is applied to the Elasticsearch Mutator
...
type featurePlan struct {
	...
	objectEdits   []func(*elasticsearchv1.Elasticsearch) error
}

// Mutator is a high-level helper for modifying Elasticsearch objects.
//
// It uses a "plan-and-apply" pattern: mutations are recorded first, then
// applied to the Elasticsearch in a single controlled pass when Apply() is called.
//
...
// NewMutator creates a new Mutator for the given Elasticsearch.
// The constructor creates the initial feature scope automatically.
func NewMutator(obj *elasticsearchv1.Elasticsearch) *Mutator {
	m := &Mutator{
		obj: obj,
	}
	m.NextFeature()
...
// EditObjectMetadata records a mutation for the Elasticsearch's own metadata.
//
...
// Edit records a mutation for the Elasticsearch itself.
//
...
//
// A nil edit function is ignored.
func (m *Mutator) Edit(edit func(*elasticsearchv1.Elasticsearch) error) {
	if edit == nil {
		return
	}
...
// Apply executes all recorded mutation intents on the underlying Elasticsearch.
//
...
// the Elasticsearch as modified by all previous features.
