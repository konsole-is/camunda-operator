...
// testObject returns a valid namespaced Cluster fixture.
func testObject() *cnpgv1.Cluster {
...
func TestBuilderBuildValidation(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			assert.Equal(t, "postgresql.cnpg.io/v1/Cluster/test-ns/test-object", res.Identity())
		})
	}
}
...
func TestExtractIntoDeclaredExtraction(t *testing.T) {
	...
	cell := concepts.NewData[string]("cnpgcluster-name")
	builder := NewBuilder(testObject())
	ExtractInto(builder, cell, func(o cnpgv1.Cluster) (string, error) {
		return o.Name, nil
	})
	...
	assert.Equal(t, "cnpgcluster-name", produced[0].Name())
