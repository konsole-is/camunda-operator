// Package cnpgcluster provides a builder and resource for managing Cluster objects.
package cnpgcluster
...
// Mutation defines a mutation that is applied to the Cluster Mutator
...
type featurePlan struct {
	...
	objectEdits   []func(*cnpgv1.Cluster) error
}

// Mutator is a high-level helper for modifying Cluster objects.
//
// It uses a "plan-and-apply" pattern: mutations are recorded first, then
// applied to the Cluster in a single controlled pass when Apply() is called.
//
...
// NewMutator creates a new Mutator for the given Cluster.
// The constructor creates the initial feature scope automatically.
func NewMutator(obj *cnpgv1.Cluster) *Mutator {
	m := &Mutator{
		obj: obj,
	}
	m.NextFeature()
...
// EditObjectMetadata records a mutation for the Cluster's own metadata.
//
...
// Edit records a mutation for the Cluster itself.
//
...
//
// A nil edit function is ignored.
func (m *Mutator) Edit(edit func(*cnpgv1.Cluster) error) {
	if edit == nil {
		return
	}
...
// Apply executes all recorded mutation intents on the underlying Cluster.
//
...
// the Cluster as modified by all previous features.
