...
// Builder is a configuration helper for creating and customizing the Keycloak Resource.
//
...
type Builder struct {
	base *generic.WorkloadBuilder[*Keycloak, *Mutator]
}

// NewBuilder initializes a new Builder with the provided Keycloak object.
//
// The Keycloak object serves as the desired base state. During reconciliation the
// framework makes the cluster's state match this base state, modified by any
// registered mutations.
//
// The provided Keycloak must have a Name set and a Namespace set, which is
// validated during the Build() call.
func NewBuilder(obj *Keycloak) *Builder {
	identityFunc := func(o *Keycloak) string {
		return fmt.Sprintf("k8s.keycloak.org/v2alpha1/Keycloak/%s/%s", o.Namespace, o.Name)
	}

	base := generic.NewWorkloadBuilder[*Keycloak, *Mutator](
		obj,
		identityFunc,
		NewMutator,
	)
...
// WithMutation registers one or more feature-based mutations for the Keycloak.
//
...
// Keycloak has reached its converged state.
//
// The default behavior uses DefaultConvergingStatusHandler, which maps the
// conditions of the Keycloak custom resource. This handler is required by the
// generic layer, so it is registered in NewBuilder and can only be replaced,
// never cleared.
func (b *Builder) WithCustomConvergeStatus(
	handler func(concepts.ConvergingOperation, *Keycloak) (concepts.AliveStatusWithReason, error),
) *Builder {
	b.base.WithCustomConvergeStatus(handler)
	return b
}

// WithCustomGraceStatus overrides how the Keycloak reports its health once the
...
func (b *Builder) WithCustomGraceStatus(
	handler func(*Keycloak) (concepts.GraceStatusWithReason, error),
) *Builder {
...
func (b *Builder) WithCustomSuspendStatus(
	handler func(*Keycloak) (concepts.SuspensionStatusWithReason, error),
) *Builder {
	b.base.WithCustomSuspendStatus(handler)
	return b
}

// WithCustomSuspendMutation defines how the Keycloak is modified when the component
// is suspended.
//
...
// Keycloak when the component is suspended.
//
// The default behavior uses DefaultDeleteOnSuspendHandler.
func (b *Builder) WithCustomSuspendDeletionDecision(handler func(*Keycloak) bool) *Builder {
	b.base.WithCustomSuspendDeletionDecision(handler)
	return b
}

// WithGuard registers a guard precondition that is evaluated before the Keycloak is
// applied during reconciliation. If the guard returns Blocked, the Keycloak and all
...
func (b *Builder) WithGuard(
	guard func(Keycloak) (concepts.GuardStatusWithReason, error),
) *Builder {
	b.base.WithGuard(generic.WrapGuard(guard))
	return b
}

// WithDataGuard declares that the Keycloak reads the given data cells and must not
...
// WithOptionalData declares that the Keycloak reads the given data cells without
...
//   - No Keycloak object was provided.
//   - The Keycloak is missing a Name.
//   - The Keycloak is missing a Namespace.
...
// ExtractInto declares that this Keycloak produces the value of cell. fn computes
// the value from a copy of the reconciled Keycloak; the framework stores it in the
// cell and marks it present, immediately after the Keycloak is applied or fetched.
...
// parameter V.
func ExtractInto[V any](b *Builder, cell *concepts.Data[V], fn func(Keycloak) (V, error)) {
