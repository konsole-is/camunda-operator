...
// Mutation defines a mutation that is applied to the Keycloak Mutator
...
type featurePlan struct {
	...
	objectEdits   []func(*Keycloak) error
}

// Mutator is a high-level helper for modifying Keycloak objects.
//
// It uses a "plan-and-apply" pattern: mutations are recorded first, then
// applied to the Keycloak in a single controlled pass when Apply() is called.
//
...
// NewMutator creates a new Mutator for the given Keycloak.
// The constructor creates the initial feature scope automatically.
func NewMutator(obj *Keycloak) *Mutator {
	m := &Mutator{
		obj: obj,
	}
	m.NextFeature()
...
// EditObjectMetadata records a mutation for the Keycloak's own metadata.
//
...
// Edit records a mutation for the Keycloak itself.
//
...
//
// A nil edit function is ignored.
func (m *Mutator) Edit(edit func(*Keycloak) error) {
	if edit == nil {
		return
	}
...
// Apply executes all recorded mutation intents on the underlying Keycloak.
//
...
// the Keycloak as modified by all previous features.
