...
// testObject returns a valid namespaced Keycloak fixture.
func testObject() *Keycloak {
	return &Keycloak{
...
func TestBuilderBuildValidation(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			assert.Equal(t, "k8s.keycloak.org/v2alpha1/Keycloak/test-ns/test-object", res.Identity())
		})
	}
}
...
func TestExtractIntoDeclaredExtraction(t *testing.T) {
	...
	cell := concepts.NewData[string]("keycloak-name")
	builder := NewBuilder(testObject())
	ExtractInto(builder, cell, func(o Keycloak) (string, error) {
		return o.Name, nil
	})
	...
	assert.Equal(t, "keycloak-name", produced[0].Name())
