...
// testObject returns a valid namespaced DatabaseConfig fixture.
func testObject() *apiv1.DatabaseConfig {
	return &apiv1.DatabaseConfig{
...
func TestBuilderBuildValidation(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			assert.Equal(t, "core.camunda.io/v1/DatabaseConfig/test-ns/test-object", res.Identity())
		})
	}
}
...
func TestExtractIntoDeclaredExtraction(t *testing.T) {
	...
	cell := concepts.NewData[string]("databaseconfig-name")
	builder := NewBuilder(testObject())
	ExtractInto(builder, cell, func(o apiv1.DatabaseConfig) (string, error) {
		return o.Name, nil
	})
	...
	assert.Equal(t, "databaseconfig-name", produced[0].Name())
