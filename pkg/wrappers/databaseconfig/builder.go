...
// Builder is a configuration helper for creating and customizing the DatabaseConfig Resource.
//
...
type Builder struct {
	base *generic.StaticBuilder[*apiv1.DatabaseConfig, *Mutator]
}

// NewBuilder initializes a new Builder with the provided DatabaseConfig object.
//
// The DatabaseConfig object serves as the desired base state. During reconciliation the
// framework makes the cluster's state match this base state, modified by any
// registered mutations.
//
// The provided DatabaseConfig must have a Name set and a Namespace set, which is
// validated during the Build() call.
func NewBuilder(obj *apiv1.DatabaseConfig) *Builder {
	identityFunc := func(o *apiv1.DatabaseConfig) string {
		return fmt.Sprintf("core.camunda.io/v1/DatabaseConfig/%s/%s", o.Namespace, o.Name)
	}

	base := generic.NewStaticBuilder[*apiv1.DatabaseConfig, *Mutator](
		obj,
		identityFunc,
		NewMutator,
	)
...
// WithMutation registers one or more feature-based mutations for the DatabaseConfig.
//
...
// WithGuard registers a guard precondition that is evaluated before the DatabaseConfig is
// applied during reconciliation. If the guard returns Blocked, the DatabaseConfig and all
...
func (b *Builder) WithGuard(
	guard func(apiv1.DatabaseConfig) (concepts.GuardStatusWithReason, error),
) *Builder {
	b.base.WithGuard(generic.WrapGuard(guard))
	return b
}

// WithDataGuard declares that the DatabaseConfig reads the given data cells and must not
...
// WithOptionalData declares that the DatabaseConfig reads the given data cells without
...
//   - No DatabaseConfig object was provided.
//   - The DatabaseConfig is missing a Name.
//   - The DatabaseConfig is missing a Namespace.
...
// ExtractInto declares that this DatabaseConfig produces the value of cell. fn computes
// the value from a copy of the reconciled DatabaseConfig; the framework stores it in the
// cell and marks it present, immediately after the DatabaseConfig is applied or fetched.
...
// parameter V.
func ExtractInto[V any](b *Builder, cell *concepts.Data[V], fn func(apiv1.DatabaseConfig) (V, error)) {
