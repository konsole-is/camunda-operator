...
// testObject returns a valid namespaced DatabaseServerConfig fixture.
func testObject() *apiv1.DatabaseServerConfig {
	return &apiv1.DatabaseServerConfig{
		ObjectMeta: metav1.ObjectMeta{Name: "test-object", Namespace: "test-ns"},
		Spec: apiv1.DatabaseServerConfigSpec{
			Engine:                    apiv1.DatabaseEnginePostgres,
			Host:                      "test-object-rw.test-ns.svc",
			Port:                      5432,
			AdminCredentialsSecretRef: apiv1.LocalCredentialsSecretRef{Name: "test-object-superuser"},
		},
	}
}
...
func TestBuilderBuildValidation(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			assert.Equal(t, "core.camunda.io/v1/DatabaseServerConfig/test-ns/test-object", res.Identity())
		})
	}
}
...
func TestExtractIntoDeclaredExtraction(t *testing.T) {
	...
	cell := concepts.NewData[string]("databaseserverconfig-name")
	builder := NewBuilder(testObject())
	ExtractInto(builder, cell, func(o apiv1.DatabaseServerConfig) (string, error) {
		return o.Name, nil
	})
	...
	assert.Equal(t, "databaseserverconfig-name", produced[0].Name())
