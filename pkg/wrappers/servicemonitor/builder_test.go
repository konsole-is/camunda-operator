...
// testObject returns a valid namespaced ServiceMonitor fixture.
func testObject() *monitoringv1.ServiceMonitor {
...
func TestBuilderBuildValidation(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			assert.Equal(t, "monitoring.coreos.com/v1/ServiceMonitor/test-ns/test-object", res.Identity())
		})
	}
}
...
func TestExtractIntoDeclaredExtraction(t *testing.T) {
	...
	cell := concepts.NewData[string]("servicemonitor-name")
	builder := NewBuilder(testObject())
	ExtractInto(builder, cell, func(o monitoringv1.ServiceMonitor) (string, error) {
		return o.Name, nil
	})
	...
	assert.Equal(t, "servicemonitor-name", produced[0].Name())
