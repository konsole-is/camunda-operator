// Package podmonitor provides a builder and resource for managing PodMonitor objects.
package podmonitor
...
// Mutation defines a mutation that is applied to the PodMonitor Mutator
...
type featurePlan struct {
	...
	objectEdits   []func(*monitoringv1.PodMonitor) error
}

// Mutator is a high-level helper for modifying PodMonitor objects.
//
// It uses a "plan-and-apply" pattern: mutations are recorded first, then
// applied to the PodMonitor in a single controlled pass when Apply() is called.
//
...
// NewMutator creates a new Mutator for the given PodMonitor.
// The constructor creates the initial feature scope automatically.
func NewMutator(obj *monitoringv1.PodMonitor) *Mutator {
	m := &Mutator{
		obj: obj,
	}
	m.NextFeature()
...
// EditObjectMetadata records a mutation for the PodMonitor's own metadata.
//
...
// Edit records a mutation for the PodMonitor itself.
//
...
//
// A nil edit function is ignored.
func (m *Mutator) Edit(edit func(*monitoringv1.PodMonitor) error) {
	if edit == nil {
		return
	}
...
// Apply executes all recorded mutation intents on the underlying PodMonitor.
//
...
// the PodMonitor as modified by all previous features.
