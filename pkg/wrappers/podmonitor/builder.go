...
// Builder is a configuration helper for creating and customizing the PodMonitor Resource.
//
...
type Builder struct {
	base *generic.StaticBuilder[*monitoringv1.PodMonitor, *Mutator]
}

// NewBuilder initializes a new Builder with the provided PodMonitor object.
//
// The PodMonitor object serves as the desired base state. During reconciliation the
// framework makes the cluster's state match this base state, modified by any
// registered mutations.
//
// The provided PodMonitor must have a Name set and a Namespace set, which is
// validated during the Build() call.
func NewBuilder(obj *monitoringv1.PodMonitor) *Builder {
	identityFunc := func(o *monitoringv1.PodMonitor) string {
		return fmt.Sprintf("monitoring.coreos.com/v1/PodMonitor/%s/%s", o.Namespace, o.Name)
	}

	base := generic.NewStaticBuilder[*monitoringv1.PodMonitor, *Mutator](
		obj,
		identityFunc,
		NewMutator,
	)
...
// WithMutation registers one or more feature-based mutations for the PodMonitor.
//
...
// WithGuard registers a guard precondition that is evaluated before the PodMonitor is
// applied during reconciliation. If the guard returns Blocked, the PodMonitor and all
...
func (b *Builder) WithGuard(
	guard func(monitoringv1.PodMonitor) (concepts.GuardStatusWithReason, error),
) *Builder {
	b.base.WithGuard(generic.WrapGuard(guard))
	return b
}

// WithDataGuard declares that the PodMonitor reads the given data cells and must not
...
// WithOptionalData declares that the PodMonitor reads the given data cells without
...
//   - No PodMonitor object was provided.
//   - The PodMonitor is missing a Name.
//   - The PodMonitor is missing a Namespace.
...
// ExtractInto declares that this PodMonitor produces the value of cell. fn computes
// the value from a copy of the reconciled PodMonitor; the framework stores it in the
// cell and marks it present, immediately after the PodMonitor is applied or fetched.
...
// parameter V.
func ExtractInto[V any](b *Builder, cell *concepts.Data[V], fn func(monitoringv1.PodMonitor) (V, error)) {
