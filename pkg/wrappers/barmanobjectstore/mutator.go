...
// Mutation defines a mutation that is applied to the ObjectStore Mutator
...
type featurePlan struct {
	...
	objectEdits   []func(*ObjectStore) error
}

// Mutator is a high-level helper for modifying ObjectStore objects.
//
// It uses a "plan-and-apply" pattern: mutations are recorded first, then
// applied to the ObjectStore in a single controlled pass when Apply() is called.
//
...
// NewMutator creates a new Mutator for the given ObjectStore.
// The constructor creates the initial feature scope automatically.
func NewMutator(obj *ObjectStore) *Mutator {
	m := &Mutator{
		obj: obj,
	}
	m.NextFeature()
...
// EditObjectMetadata records a mutation for the ObjectStore's own metadata.
//
...
// Edit records a mutation for the ObjectStore itself.
//
...
//
// A nil edit function is ignored.
func (m *Mutator) Edit(edit func(*ObjectStore) error) {
	if edit == nil {
		return
	}
...
// Apply executes all recorded mutation intents on the underlying ObjectStore.
//
...
// the ObjectStore as modified by all previous features.
